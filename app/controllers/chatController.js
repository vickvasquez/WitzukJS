...
function getUsersFromHs(hs) {
	debugger;	
}

var chatControllers = function(io) {

	//var io = socket.listen(app);
//Obtener las salas disponibles para 
	...
	io.sockets.on('connection', function(socket) {
		...
		socket.on('usuario', function(data) {
			...
			if (usuario === null || usuario === undefined || usuario === "") {
				console.log('Error: el usuario no puede ser: ' + usuario);
				return;
			} else {
		...
		socket.on('general', function(msj) {
			fecha = new DB.fecha();
			hora = fecha.hora();
			DB.msjsala(socket.usuarios, msj, socket.sala, function(err, res) {
				socket.emit('sms', socket.usuarios, msj, hora);
				socket.broadcast.emit('all', socket.usuarios, msj, hora);
		...
		var salasss = function() {
			...
			for (var i = 0; i < sala.length; i++) {
				//	debugger;
				//	console.log('Sala '+sala[i]+" :"+ io.nsps['/']);
